...
void Mapper_01::cpu_write(Uint16 address, Uint8 value) {
    if (address >= 0x0000 && address <= 0x1FFF) {
        std::cout << "[WARN] Write to mapper 01 enable/disable ram occurred: [" << hex(address, 4) << "] = " << hex(value, 2) << std::endl;
    } else if (address >= 0x2000 && address <= 0x3FFF) {
        selected_rom_bank = value == 0 ? 1 : value & 0x1F;
    } else if (address >= 0x4000 && address <= 0x5FFF) {
        std::cout << "[WARN] Write to mapper 01 ram bank number occurred: [" << hex(address, 4) << "] = " << hex(value, 2) << std::endl;
    } else if (address >= 0x6000 && address <= 0x7FFF) {
        std::cout << "[WARN] Write to mapper 01 banking mode select occurred: [" << hex(address, 4) << "] = " << hex(value, 2) << std::endl;
    }
}
